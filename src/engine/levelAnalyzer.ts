...
import { positionEquals, createEmptyGrid, isWalkable } from './GameEngine';
import { getCell, setCell, isObstacle } from './gridEditor';
...
export function bfsShortestPath(
  ...
  while (queue.length > 0) {
    ...
    for (const dir of directions) {
      ...
      if (avoidPits && cell === 'pit') continue;
    ...
    for (let x = 0; x < width; x++) {
      ...
      while (queue.length > 0) {
        ...
        for (const dir of directions) {
          ...
          if (avoidPits && nextCell === 'pit') continue;
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      for (const dir of directions) {
        ...
        if (neighbor !== 'wall' && (!avoidPits || neighbor !== 'pit')) {
          openNeighbors++;
        }
      }
